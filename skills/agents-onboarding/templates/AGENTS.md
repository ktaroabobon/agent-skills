...
.claude/settings.json     # hooks の配線
```
...
`.agents/hooks/protect-generated-files.sh`（PreToolUse）が、生成物・稼働物・secrets への Edit/Write を機械的にブロックする。
...
- **`git push --force`, `git reset --hard` を自主判断で実行**（ユーザ確認必須）
