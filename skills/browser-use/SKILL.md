...
[browser-use/browser-use](https://github.com/browser-use/browser-use) が配布している公式スキル `skills/browser-use/SKILL.md` の写し。本文は upstream の英語のまま置き、先頭にこの節だけを足している。翻訳しないのは、CLI の挙動は upstream のバージョンに従うので、原文のまま差分を追えるほうが安全なため。

## このリポジトリでの扱い

- **前提**: `browser-use` CLI がインストール済みで PATH にあること。入れ方と接続トラブルは https://github.com/browser-use/browser-harness/blob/main/install.md
- **Claude Code** に組み込みのブラウザツール(Browser / Chrome 連携)がある環境では、それで足りるならそちらを使う。このスキルは、CLI を共通にしたいときや Codex など組み込みツールが無い環境向け
- **更新**: upstream は頻繁に変わる。`metadata.upstream-commit` より新しいコミットがあれば、差分を見て本文を差し替え、`metadata` を更新する

  ```bash
  diff <(gh api "repos/browser-use/browser-use/contents/skills/browser-use/SKILL.md" -H "Accept: application/vnd.github.raw" | sed -n '/^# Browser Use$/,$p') \
       <(sed -n '/^# Browser Use$/,$p' skills/browser-use/SKILL.md)
  ```

  写しではなく upstream を直接入れるなら `gh skill install browser-use/browser-use browser-use`

以下、upstream 本文(commit fa57d34、2026-08-16)。
